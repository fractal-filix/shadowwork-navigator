...
【厳守事項】機密情報の取り扱い（必須）
- 機密情報が記載されている可能性があるファイル（例: `.dev.vars` / `.env*` / `*.pem` / `*.key` / `*-secret*` / `secrets.*` / 資格情報を含む設定ファイル等）は、Copilot が内容を読み込まないこと（ツールで開かない／検索対象にしない）。
- 上記のファイルはコミットしないこと（追加・変更の提案も原則しない）。
- 誤って読み込んだ可能性がある場合は、以降の作業を止めてユーザーへ報告し、キーのローテーション等の対処を優先する。
...
- PR テンプレ必須: 概要 / 目的 / 変更点（要約） / 検証（自動テストの場合はコマンド） / 影響範囲 / チェックリスト（タスクを別管理の場合はその旨を明記）。
...
ツール
- git kraken は未認証なので gh を使用してください。
