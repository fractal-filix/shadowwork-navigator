...
- **payload 仕様（現行）**: `schema='rag_chunk_v1'`, `user_id`, `thread_id`, `message_id`, `chunk_no`, `text` を必須とし、`client_message_id` は任意とする
- **point id 仕様**: `${message_id}#${chunk_no}` を採用し、同一メッセージの再アップサートを上書き可能にする
