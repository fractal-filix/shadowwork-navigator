...
- **ベクトル検索（RAG）用の派生データ管理**
  - メッセージ本文（暗号文）とは別に、検索用チャンクをベクトルDBへ保存する（後述）
  - ベクトルDBは検索インデックスであり、D1の本文暗号文とは責務が異なる
- **運用に必要な最小機能**
...
- **会員登録 / ログイン UI**（Supabase Auth などの外部認証基盤側の体験）
...
flowchart LR
  ...
  SB[Supabase Auth] -. Session / access token .-> FE
...
- RAG（ベクトル検索）を行う場合、**検索用チャンクは本文とは別にベクトルDBへ保存**する（チャンクはやむを得ず平文を含みうる）
...
sequenceDiagram
  ...
  loop 進行（複数回）
    ...
    Note over FE,API: （RAG導入時）チャンク化→ベクトル化→ベクトルDBへ保存
    FE->>API: rag chunk upsert（チャンク + メタ）
    API->>VDB: upsert（embedding + payload）
    API-->>FE: 保存結果
  end
